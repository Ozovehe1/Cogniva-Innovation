      ...
      {navItems.map(({ href, icon, label }) => {
        ...
        return (
          <Link key={href} href={href} onClick={onNav}
            ...
            <span className="w-4 flex items-center justify-center flex-shrink-0" style={{ color: active ? '#34D399' : '#52525B' }}>{icons[icon] ?? icon}</span>
            <span>{label}</span>
          </Link>
        )
      })}
    ...
    <div className="flex h-screen" style={{ background: '#09090B' }}>
      ...
      {open && (
        <div className="fixed inset-0 z-50 md:hidden">
          ...
          <aside className="absolute left-0 top-0 h-full w-64 flex flex-col" style={{ background: '#0A0F0C', borderRight: '1px solid rgba(255,255,255,0.06)' }}>
            <div className="flex items-center justify-between px-4 py-4" style={{ borderBottom: '1px solid rgba(255,255,255,0.06)' }}>
              <div className="flex items-center gap-2">
                <div className="w-6 h-6 rounded-md bg-emerald-700 flex items-center justify-center text-white text-xs font-bold">G</div>
                <span className="text-white font-semibold text-sm">GeniusMap</span>
              </div>
      ...
      <div className="flex-1 flex flex-col min-w-0 overflow-hidden">
        {/* Mobile top bar */}
        <header className="flex md:hidden items-center justify-between px-4 py-3 flex-shrink-0" style={{ background: '#0A0F0C', borderBottom: '1px solid rgba(255,255,255,0.06)' }}>
          <div className="flex items-center gap-2">
            <div className="w-6 h-6 rounded-md bg-emerald-700 flex items-center justify-center text-white text-xs font-bold">G</div>
            <span className="text-white font-semibold text-sm">GeniusMap</span>
          </div>
